...
func _handle_inputs() -> void:
	if Input.is_action_just_pressed("jump") and is_on_floor():  # Changed input
		velocity.y = JUMP_VELOCITY
		state = States.JUMPING
		ground_y_position = global_position.y  # Update ground level on jump

	var direction := Input.get_axis("move_left", "move_right")  # Changed input
	if direction:
...
func _handle_ball_spawn() -> void:
	if Input.is_action_just_pressed("spawn_ball_L"):  # Changed input
		_spawn_ball()
...
func _handle_ball_hit() -> void:
	...
	if Input.is_action_just_pressed("trigger_action_ball"):  # Changed input
...
func _hit_ball(ball: RigidBody2D) -> void:
	if not is_on_floor():
		# SPIKE: Sharp downward angle (mirrored for left player)
		if _is_in_zone(zone_1):
			var spike_direction = Vector2.from_angle(deg_to_rad(20))  # Mirrored
			ball.apply_central_impulse(spike_direction * SPIKE_FORCE)
		elif _is_in_zone(zone_2_L):  # Changed to zone_2_L
			var spike_direction = Vector2.from_angle(deg_to_rad(-20))  # Mirrored
			ball.apply_central_impulse(spike_direction * SPIKE_FORCE)
	else:
		...
		if _is_in_zone(zone_1):
			# Zone 1: Steep angle (mirrored)
			hit_direction = Vector2.from_angle(deg_to_rad(-80))  # Mirrored
			ball.apply_central_impulse(hit_direction * NORMAL_HIT_FORCE_for_Z1)
		elif _is_in_zone(zone_2_L):  # Changed to zone_2_L
			# Zone 2: Normal angle (mirrored)
			hit_direction = Vector2.from_angle(deg_to_rad(-60))  # Mirrored
			ball.apply_central_impulse(hit_direction * NORMAL_HIT_FORCE_for_Z2)
		else:
			# Default: Normal angle (mirrored)
			hit_direction = Vector2.from_angle(deg_to_rad(-60))  # Mirrored
			ball.apply_central_impulse(hit_direction * NORMAL_HIT_FORCE_for_Z2)
